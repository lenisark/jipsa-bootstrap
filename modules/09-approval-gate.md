...
> `저장폴더` 명령은 `add_dirs`(접근 허용)와 `output_dir`(기본 저장 위치)을 함께 설정한다. `output_dir`은 시스템 프롬프트에 "새 파일은 여기 저장"으로 주입돼, "파일 만들어줘"가 그 폴더에 떨어진다. 구글 드라이브 같은 마운트 경로(`G:\공유 드라이브\...`)도 가능.
